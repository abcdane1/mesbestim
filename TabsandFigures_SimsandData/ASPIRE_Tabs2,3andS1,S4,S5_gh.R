...
data_aspire_fun_dfme<-function(m,csl,csu,sdvalout,sdvalsel,sdvalme,diffx=T,wdoc=F,
  ...
  #induce within group correlation via random intercept (like in glmm)
...
nmetrics<-26 #need to change for different scenarios
...
##table 1 scenarios, but later added an error function
...
##table s2 scenarios
# table1scen<-data.frame(nc=rep(30,16),csl=rep(rep(c(100,500),each=2),4),csu=rep(rep(c(300,1000),each=2),4),wdoc=T,
#                        icc=c(rep(rep(c(.01,.1),each=4),2)),icc_me=0,diffx=c(rep(F,8),rep(T,8)),diffx_mod=rep(c(T,F),8))

##table s3 scenarios
table1scen<-data.frame(nc=rep(30,16),csl=rep(rep(c(100,500),each=2),4),csu=rep(rep(c(300,1000),each=2),4),wdoc=F,
                       icc=c(rep(rep(c(.01,.1),each=4),2)),icc_me=c(rep(rep(c(.01,.1),each=4),2)),diffx=c(rep(F,8),rep(T,8)),diffx_mod=rep(c(T,F),8))
...
bootvec<-rep(F,16) 
#bootvec<-ifelse(scenariomat$csl==100,T,F) #only need for table 1
...
for(i in 1:(nrow(scenariomat)/2)){
  ...
  #mean of estimates
  finalw<-rowMeans(lw[-c(seq(10,25,by=3),seq(11,26,by=3)),],na.rm=T) 
  
  #sum of convergence fails
  convfailw<-rowSums(is.na(lw[-c(seq(10,25,by=3),seq(11,26,by=3)),]),na.rm=T)
  
  #true ate (mean of difference of means)
  true_atew<-finalw[7]
  ...
  finalwo<-rowMeans(lwo[-c(seq(10,25,by=3),seq(11,26,by=3)),],na.rm=T) 
  ...
  if(i %% 1 == 0){
    #aspiresimsv2bothwdoc, file name table s2
    filename<-paste("aspiresimsv2bothwmenodoc",i,".csv",sep="")
