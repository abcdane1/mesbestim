...
data_aspire_fun_dfme_nb<-function(m,msizel,sdvalout,sdvalsel,sdvalme,diffx=T,wdoc=F,
  ...
  if(msizel==F){
    cs<-rnbinom(m,2,.02)+102
  }else{
    cs<-rnbinom(m,3,.01)+453}
...
wrapper_aspire_funerr_v3<-function(sim,m,msizel,sdvalout,sdvalsel,sdvalme,xlabs,clabs,diffx,wdoc,
                                   sdvaldout,sdvaldsel,sdvaldme,dlb,dub,cor,ind,boot){
  #data generation
  dfsim<-data_aspire_fun_dfme_nb(m=m,msizel=msizel,sdvalout=sdvalout,sdvalsel=sdvalsel,sdvalme=sdvalme,diffx=diffx,
...
##table nb scenarios, error function added
tablenbscen<-data.frame(nc=rep(30,16),msizel=rep(rep(c(F,T),each=2),4),wdoc=F,
                        icc=c(rep(rep(c(.01,.1),each=4),2)),icc_me=0,diffx=c(rep(F,8),rep(T,8)),diffx_mod=rep(c(T,F),8))
scenariomat<-tablenbscen
...
#bootvec<-rep(F,16)
bootvec<-ifelse(scenariomat$msizel==F,T,F) #make smaller size boot only 

#obtaining output of simulations
for(i in 1:(nrow(scenariomat)/2)){
  l<-parallel::mcmapply(wrapper_aspire_funerr_v3,sim=1:nsim,
                        MoreArgs=list(m=scenariomat[2*i-1,"nc"],msizel=scenariomat[2*i-1,"msizel"],
                                      sdvalout=sdvals[2*i-1],sdvalsel=sdvals[2*i-1],sdvalme=sdvalsme[2*i-1],diffx=scenariomat[2*i-1,"diffx"],
  ...
  if(i %% 1 == 0){
    filename<-paste("aspiresimsv2_nb",i,".csv",sep="")
