...
data_aspire_fun_dfme_f2<-function(m,csl,csu,small_sel=T,small_valerr=T,sdvalout,sdvalsel,sdvalme,
  ...
  for(i in 1:m){
    ...
    if(small_valerr==T){
      #overall <10%
      ps1<-1/(1+exp(2.75*(1-y1val)-3*y1val+.35*valx1+.25*valx2-.15*valx3+.1*valx4-bimei-bidmei)) 
      ps0<-1/(1+exp(2*(1-y0val)-2*y0val+.55*valx1+.35*valx2-.15*valx3+.1*valx4-bimei-bidmei))}else{
        
      #overall >40%
      ps1<-1/(1+exp(.2*(1-y1val)-.75*y1val+.35*valx1+.25*valx2-.15*valx3-bimei-bidmei)) 
    ...
    if(small_sel==T){
      #20%
      pselect1<-1/(1+exp(.15+.15*y1val+.75*valx1+.75*valx2+.75*valx3-.15*valx4-bisi-bidsi)) 
      pselect0<-1/(1+exp(.25-.15*y0val+.75*valx1+.75*valx2+.75*valx3-.15*valx4-bisi-bidsi))}else{
        
      #40%
      pselect1<-1/(1+exp(-.45+.15*y1val+.5*valx1+.5*valx2+.5*valx3-.1*valx4-bisi-bidsi)) 
      pselect0<-1/(1+exp(-.7-.15*y0val+.5*valx1+.5*valx2+.5*valx3-.1*valx4-bisi-bidsi))}
    
    #observed selection probabilities under treatment
...
##SSW estimator 
estimator_aspire_fun_v3<-function(df,xlabs,clabs,varprint=T,diffx=T,cor=T,ind=T,hom=F){
  N<-nrow(df)
  #validation subset
  dfv<-df[which(df$v==1),]
  ...
  #if classification model depends on at least treatment arm as well as y
  if(hom==F){
  ...
  #if classification model only depends on y
  if(hom==T){
    p111hat<-rep(sum(dfv$ys*dfv$y)/sum(dfv$y),N)
    p101hat<-rep(sum(dfv$ys*(1-dfv$y))/sum((1-dfv$y)),N)
    p110hat<-p111hat
    p100hat<-p101hat
    phat<-mean(df$a)
  }
  ...
  #just print SSW estimators
  if(varprint==F){
    return(c("Estimator"=finalest,"P1"=u1hat,"P0"=u0hat))
  }else{
    
    #note: not used in this figure
...
estimator_aspire_valid<-function(df,xlabs,varprint=T,cor=T){
  ...
  #selection model (correctly specified for data generating mechanism, less gs outcome)
  ...
  #IPSW estim treatment 1 using selection subset
  u1hat<-sum((dfv$y*dfv$a)/(fitted_v_select*phat))/N
  
  #IPSW estim treatment 0 using selection subset
  u0hat<-sum((dfv$y*(1-dfv$a))/(fitted_v_select*(1-phat)))/N
  
  #IPSW estimate of ATE 
  finalest<-u1hat-u0hat
  truth<-mean(df$y1-df$y0)
  ...
  if(varprint==F){
    ...
    #note: not used for this figure
    #for asymptotic variance estimation, with sandwich expression
    ...
    #estimating equations with cluster index 
    for(i in 1:m){
...
wrapper_aspire_funerr_v3_f2<-function(sim,m,csl,csu,small_sel,small_valerr,
  ...
  #selection and missclassification specs
  ...
  ##ssw estim
  l1<-check_fail(func=estimator_aspire_fun_v3,df=dfsim,xlabs=xlabs,clabs=clabs,varprint=F,diffx=T,cor=cor,ind=ind,hom=F)
  
  ##ssw estim with homogeneous misclassification
  l2<-check_fail(func=estimator_aspire_fun_v3,df=dfsim,xlabs=xlabs,clabs=clabs,varprint=F,diffx=T,cor=cor,ind=ind,hom=T)
  
  ##ipsw estim
  l3<-check_fail(func=estimator_aspire_valid,df=dfsim,xlabs=c(xlabs,clabs),varprint=F,cor=cor)
  
  ##sso estim
  l4<-c(mean(dfsim$ys*dfsim$a/mean(dfsim$a))-mean(dfsim$ys*(1-dfsim$a)/(1-mean(dfsim$a))),
        mean(dfsim$ys*dfsim$a/mean(dfsim$a)),mean(dfsim$ys*(1-dfsim$a)/(1-mean(dfsim$a))))
  
  estimate_opts<-c(truth,l1,l2,l3,l4)
  names(estimate_opts)<-c("TrueATE","Proposed_Estim","Proposed_P1","Proposed_P0",
                          "Proposed_Estim_Hom","Proposed_P1_Hom","Proposed_P0_Hom",
                          "IPSW_Estim","IPSW_P1","IPSW_P0",
                          "SSOnly_Estim","SSOnly_P1","SSOnly_P0")
...
#parallelization over scenarios
for(i in 1:nrow(scenariomat)){
  ...
  if(i %% 1 == 0){
    filename<-paste("aspiref4homcomp",i,".rds",sep="")
